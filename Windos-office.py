...
def conectar_google_sheets():
    # Extraemos los datos del secret
    gcp_secrets = st.secrets["gcp_service_account"]
    
    # Limpiamos la llave privada asegurando los saltos reales
    priv_key = gcp_secrets["private_key"].replace("\\n", "\n")
    ...
    # Creamos un archivo JSON temporal físico. Esto evita por completo el error de criptografía de los secrets.
    ...
    try:
        ...
        sheet_connection = client.open_by_key("1TxukcYhPDoCKrbqhzbUBKFLauvTRBlwrXq5IH2m0Zs")
    finally:
        # Limpiamos el archivo temporal del servidor
        if os.path.exists(temp_file_path):
            os.remove(temp_file_path)
            
    return sheet_connection
